...
// client loader - in real project server laoder would be better
export async function loader({ request, params }: Route.LoaderArgs) {
  // const res = await fetch(`http://localhost:8000/projects/${params.id}`);

  // documentId is passed in as params.id
  const { id } = params;

  // bring in the project as an array called data
  // filter is used to select the project with the specified 'id'
  ...
  // The data array has only a single element ('0')
